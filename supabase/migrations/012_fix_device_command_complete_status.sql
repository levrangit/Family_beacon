-- ============================================================
-- FIX DEVICE COMMAND COMPLETE STATUS TYPE
-- Recreate the RPC using public.command_status instead of text.
-- ============================================================
...
begin
    ...
    -- Only final states are allowed.
    if target_status not in (
        'completed'::public.command_status,
        'failed'::public.command_status
    ) then
        raise exception 'Invalid command status';
    end if;

    -- Device may complete only its own executing command.
